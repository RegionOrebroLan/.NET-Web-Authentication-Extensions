...
namespace RegionOrebroLan.Web.Authentication.DirectoryServices
{
	public interface IActiveDirectory
	{
		...
		/// <param name="filter"><![CDATA[The filter for the user. It is possible to pass a filter that will give multiple hits. Eg. "userPrincipalName=first-name.last-name@example.org" or "|(custom-id=first-id)(custom-id=second-id)". The filter will internally be used as "(&(objectClass=person)(objectClass=user)({filter}))".]]></param>
		/// <returns></returns>
